// phone.jsx — demo phone frame (eticaret green)
function PhoneStatusBar({ tint = 'dark', bg = 'transparent', time = '9:41' }) {
  const c = tint === 'light' ? '#fff' : '#0B1F18';
  return (
    <div style={{
      display: 'flex', alignItems: 'center', justifyContent: 'space-between',
      padding: '11px 22px 6px', background: bg, position: 'relative', zIndex: 8, flexShrink: 0,
    }}>
      <span style={{ fontWeight: 700, fontSize: 13, color: c, fontFamily: 'var(--demo-display), sans-serif', letterSpacing: '.02em' }}>{time}</span>
...
function PhoneFrame({ children, width = 300, statusBg = '#fff', statusTint = 'dark', time = '9:41', glow }) {
  const ratio = 2.075;
  ...
  return (
    <div style={{
      ...
      background: 'linear-gradient(150deg,#0B1F18,#1A3D30)',
      boxShadow: glow
        ? `0 30px 70px ${glow}, 0 0 0 1px rgba(0,0,0,0.2)`
        : '0 24px 60px rgba(11,31,24,0.28), 0 0 0 1px rgba(0,0,0,0.2)',
    }}>
      <div style={{
        ...
        <div style={{
          ...
          width: width * 0.3, height: width * 0.092, borderRadius: 99, background: '#06140f', zIndex: 20,
        }} />
...
function TabBar({ items, active, onChange, accent = '#008060' }) {
  return (
    <div style={{
      display: 'flex', justifyContent: 'space-around', alignItems: 'center',
      padding: '9px 6px 16px', background: 'rgba(255,255,255,0.96)',
      borderTop: '1px solid rgba(11,31,24,0.06)', backdropFilter: 'blur(8px)', flexShrink: 0,
    }}>
      {items.map((it, i) => {
        const on = i === active;
        return (
          <button key={i} type="button" onClick={() => onChange && onChange(i)} style={{
            display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 3,
            color: on ? accent : '#9AADA3', flex: 1, padding: 0,
          }}>
            <Icon name={it.icon} size={21} stroke={on ? accent : '#9AADA3'} fill={on ? accent : 'none'} />
            <span style={{ fontSize: 9.5, fontWeight: 700, fontFamily: 'var(--demo-display), sans-serif' }}>{it.label}</span>
          </button>
        );
      })}
    </div>
...
function Icon({ name, size = 22, stroke = '#000', fill = 'none', sw = 1.9 }) {
  ...
  const paths = {
    ...
    camera: <><path d="M4 8h3l2-2h6l2 2h3v11H4V8Z" {...p}/><circle cx="12" cy="13.5" r="3.5" {...p}/></>,
    globe: <><circle cx="12" cy="12" r="9" {...p}/><path d="M3 12h18M12 3c3 3.5 3 14.5 0 18M12 3c-3 3.5-3 14.5 0 18" {...p}/></>,
    sync: <><path d="M21 12a9 9 0 1 1-2.6-6.3" {...p}/><path d="M21 3v6h-6" {...p}/></>,
    zap: <path d="M13 2 4 14h7l-1 8 9-12h-7l1-8Z" {...f}/>,
  };
  return <svg width={size} height={size} viewBox="0 0 24 24">{paths[name] || null}</svg>;
}

const ACCENT = '#008060';
const wrapStyle = { flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, background: '#F7FBF8', overflow: 'hidden' };
const scrollStyle = { flex: 1, overflowY: 'auto', WebkitOverflowScrolling: 'touch' };

Object.assign(window, { PhoneFrame, PhoneStatusBar, TabBar, Icon, ACCENT, wrapStyle, scrollStyle });
